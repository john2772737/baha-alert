...
const TestControlCard = ({ Icon, title, sensorKey, dbValue, status, onToggle, isActive }) => (
    <div className={`p-5 rounded-xl border shadow-md flex flex-col justify-between transition-all duration-500 
        ...
        <div className="bg-slate-900/50 p-3 rounded-lg mb-4 flex justify-between items-center border border-slate-700/50">
            <span className="text-slate-500 text-sm font-mono">Live Result:</span>
            <div className="text-right">
                <div className={`font-bold text-lg transition-all duration-300 leading-none ${dbValue && dbValue !== 'Waiting...' ? 'text-white scale-105' : 'text-slate-600'}`}>
                    {dbValue || '--'}
                </div>
                {/* New Status Label */}
                <div className={`text-[10px] font-bold uppercase tracking-wider mt-1 ${
                    !status || status === '--' ? 'text-slate-600' : 
                    status.includes('High') || status.includes('Flood') || status.includes('Storm') ? 'text-red-400' :
                    status.includes('Low') || status.includes('Dry') ? 'text-yellow-400' :
                    'text-emerald-400'
                }`}>
                    {status || 'Unknown'}
                </div>
...
const ModeView = ({ mode, setMode, liveData, fetchError, refs, percents }) => {
    ...
    // Note: We removed the strict SLEEP lockout here so we can show the dashboard overlay below

    // --- COMBINED AUTO & SLEEP VIEW ---
    if (mode === 'Auto' || mode === 'Sleep') {
        ...
        return (
            <div className="relative animate-fadeIn">
                {/* SLEEP OVERLAY: Only visible when isSleep is true */}
                {isSleep && (
                    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-slate-900/70 backdrop-blur-sm rounded-2xl border border-indigo-500/30 transition-all duration-500">
                        <div className="p-6 bg-slate-800 rounded-full shadow-2xl shadow-indigo-500/20 border border-slate-700 mb-6 animate-pulse">
                            <MoonIcon className="w-20 h-20 text-indigo-400" />
                        </div>
                        <h2 className="text-4xl font-black text-white tracking-tight mb-2">DEVICE SLEEPING</h2>
                        <div className="flex items-center gap-2 bg-slate-800/80 px-4 py-2 rounded-lg border border-slate-700 text-slate-300 font-mono text-sm">
                            <ActivityIcon className="w-4 h-4 text-indigo-400" />
                            <span>System paused to save power</span>
                        </div>
                        <p className="text-slate-400 mt-4 text-sm max-w-xs text-center">
                            Background monitoring is active. Press the physical button to wake.
                        </p>
                    </div>
                )}

                {/* DASHBOARD CONTENT: Blurred and disabled if sleeping */}
                <div className={`transition-all duration-500 ${isSleep ? 'opacity-25 pointer-events-none filter grayscale contrast-125' : ''}`}>
                    {fetchError && !isSleep && (
    ...
    if (mode === 'Maintenance') {
        // Helper to resolve status text from raw dbValues using your existing utils
        const resolveStatus = (type, val) => {
            if (val === null || val === undefined || val === 'Waiting...') return '--';
            const num = parseFloat(val);
            if (isNaN(num)) return '--';

            try {
                switch(type) {
                    case 'rain': return getRainStatus(num).status;
                    case 'soil': return getSoilStatus(num).status;
                    case 'water': return getWaterTankStatus(num, 0).status; // Assuming val is %
                    case 'pressure': return getPressureStatus(num).status;
                    default: return '--';
                }
            ...
            <section className="space-y-6 animate-fadeIn">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <TestControlCard 
                        ...
                        status={resolveStatus('rain', dbValues.rain)}
                    ...
                    <TestControlCard 
                        ...
                        status={resolveStatus('soil', dbValues.soil)}
                    ...
                    <TestControlCard 
                        ...
                        status={resolveStatus('water', dbValues.water)}
                    ...
                    <TestControlCard 
                        ...
                        status={resolveStatus('pressure', dbValues.pressure)}
