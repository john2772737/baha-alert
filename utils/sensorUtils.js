...
export const getFormattedTime = () => new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
...
export const STATE_MAPPINGS = {
    ...
    // Tank max depth 50cm
    waterTank: (distanceCM) => (distanceCM > 100 ? 0 : clamp(100.0 - (distanceCM / 50.0) * 100.0)) 
};
...
export const getSoilStatus = (percent) => {
    ...
    if (percent < 30) return { reading: 'Low', status: 'STATUS: Slightly Damp', className: 'text-yellow-400 font-bold' };
    return { reading: 'Moist', status: 'STATUS: Normal', className: 'text-emerald-400 font-bold' };
};

export const getWaterTankStatus = (percent, distance) => {
    if (distance >= 400) return { reading: 'Error', status: 'SENSOR ERROR', className: 'text-red-500 font-black animate-pulse' };
